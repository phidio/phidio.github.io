...
CallStatistics.prototype.initialize = function () {
    ...
    this.callStats.initialize(AppID, AppSecret, this.localName, initCallback, function (e) {
        console.log('stats', e);
    });
};
...
ChatView.prototype.addMessage = function (sender, message) {
    this.msgBodyEl.append('<div class="message-bubble ' + (sender ? 'send' : 'received') + '">' + '<div class="back">' + '<span class="author">' + (sender ? 'You' : 'Stranger') + '</span>' + '<div class="time">' + this.getTime() + '</div>' + '<div class="message">' + message + '</div>' + '</div>' + '</div>');
...
module.exports = {
  getCameraAccess: function getCameraAccess(callback) {
  ...
  createSessionObject: function createSessionObject(success, error, options) {
  ...
  call: function call(success, error, options) {
    if (sessions[options[0].sessionKey]) sessions[options[0].sessionKey].call();
  },
  receiveMessage: function receiveMessage(success, error, options) {
    if (sessions[options[0].sessionKey]) sessions[options[0].sessionKey].receiveMessage(JSON.parse(options[0].message));
  },
  renegotiate: function renegotiate(success, error, options) {
  ...
  disconnect: function disconnect(success, error, options) {
    var session = sessions[options[0].sessionKey];
    if (session) {
      session.disconnect(true);
    }
  },
  setVideoView: function setVideoView(success, error, options) {
    ...
    if (videoConfig.local) {
      if (!localVideoView) {
        ...
        if (!localVideoTrack) {
          ...
          if (MediaStream && !navigator.mozGetUserMedia) {
            // if mediastream supported & not FF
        ...
        if (MediaStream && !navigator.mozGetUserMedia) document.body.appendChild(localVideoView);
      } else {
        refreshLocalVideoView();
  ...
  hideVideoView: function hideVideoView(success, error, options) {
    localVideoView.style.display = 'none';
    remoteVideoViews.forEach(function (remoteVideoView) {
      remoteVideoView.style.display = 'none';
    });
  },
  showVideoView: function showVideoView(success, error, options) {
...
function refreshVideoContainer() {
  ...
  var y = getCenter(actualRows, videoSize, videoConfig.containerParams.size[1]) + videoConfig.containerParams.position[1];

  var videoViewIndex = 0;

  for (var row = 0; row < rows && videoViewIndex < n; row++) {
    var x = videoConfig.containerParams.position[0] + getCenter(row < rows - 1 || n % rows === 0 ? videosInRow : n - (Math.min(n, videoViewIndex + videosInRow) - 1), videoSize, videoConfig.containerParams.size[0]);
...
function scaleToFill(event) {
  ...
  function refreshTransform() {
    ...
    var adjustmentRatio = widthIsLargerThanHeight ? actualRatio / targetRatio : targetRatio / actualRatio;
...
},{"./adapter":4}],4:[function(require,module,exports){
//init vars
'use strict';
...
AdapterJS.renderNotificationBar = function (text, buttonText, buttonLink, openNewTab, displayRefreshBar) {
  ...
  c.document.open();
  c.document.write('<span style="display: inline-block; font-family: Helvetica, Arial,' + 'sans-serif; font-size: .9rem; padding: 4px; vertical-align: ' + 'middle; cursor: default;">' + text + '</span>');
  if (buttonText && buttonLink) {
    ...
    AdapterJS.addEvent(c.document.getElementById('okay'), 'click', function (e) {
      if (!!displayRefreshBar) {
        AdapterJS.renderNotificationBar(AdapterJS.TEXT.EXTENSION ? AdapterJS.TEXT.EXTENSION.REQUIRE_REFRESH : AdapterJS.TEXT.REFRESH.REQUIRE_REFRESH, AdapterJS.TEXT.REFRESH.BUTTON, 'javascript:location.reload()');
      }
...
checkMediaDataChannelSettings = function (peerBrowserAgent, peerBrowserVersion, callback, constraints) {
  ...
  var isPeerFirefoxInterop = peerBrowserAgent === 'firefox' && (peerBrowserVersion ? peerBrowserVersion > 30 : false);
...
checkIceConnectionState = function (peerId, iceConnectionState, callback) {
  ...
  if (!AdapterJS._iceConnectionFiredStates[peerId] || iceConnectionState === AdapterJS._iceConnectionStates.disconnected || iceConnectionState === AdapterJS._iceConnectionStates.failed || iceConnectionState === AdapterJS._iceConnectionStates.closed) {
  ...
  if (AdapterJS._iceConnectionFiredStates[peerId].indexOf(iceConnectionState) < 0) {
    ...
    if (iceConnectionState === AdapterJS._iceConnectionStates.connected) {
      setTimeout(function () {
        AdapterJS._iceConnectionFiredStates[peerId].push(AdapterJS._iceConnectionStates.done);
...
RTCSessionDescription = typeof RTCSessionDescription === 'function' ? RTCSessionDescription : null;

// Creates RTCIceCandidate object for Plugin Browsers
RTCIceCandidate = typeof RTCIceCandidate === 'function' ? RTCIceCandidate : null;
...
if (navigator.mozGetUserMedia) {
  ...
  if (webrtcDetectedBrowser === 'chrome' && webrtcDetectedVersion >= 31 || webrtcDetectedBrowser === 'opera' && webrtcDetectedVersion >= 20) {
    webrtcDetectedDCSupport = 'SCTP';
  } else if (webrtcDetectedBrowser === 'chrome' && webrtcDetectedVersion < 30 && webrtcDetectedVersion > 24) {
  ...
  AdapterJS.WebRTCPlugin.injectPlugin = function () {
    ...
    if (webrtcDetectedBrowser === 'IE' && webrtcDetectedVersion <= 10) {
      ...
      AdapterJS.WebRTCPlugin.plugin.innerHTML = '<object id="' + AdapterJS.WebRTCPlugin.pluginInfo.pluginId + '" type="' + AdapterJS.WebRTCPlugin.pluginInfo.type + '" ' + 'width="1" height="1">' + '<param name="pluginId" value="' + AdapterJS.WebRTCPlugin.pluginInfo.pluginId + '" /> ' + '<param name="windowless" value="false" /> ' + '<param name="pageId" value="' + AdapterJS.WebRTCPlugin.pageId + '" /> ' + '<param name="onload" value="' + AdapterJS.WebRTCPlugin.pluginInfo.onload + '" />' + '<param name="tag" value="' + AdapterJS.WebRTCPlugin.TAGS.NONE + '" />' + (
      // uncomment to be able to use virtual cams
      AdapterJS.options.getAllCams ? '<param name="forceGetAllCams" value="True" />' : '') + '</object>';
      ...
      AdapterJS.WebRTCPlugin.plugin = document.getElementById(AdapterJS.WebRTCPlugin.pluginInfo.pluginId);
    } else {
      // Load Plugin
  ...
  AdapterJS.WebRTCPlugin.defineWebRTCInterface = function () {
    ...
    RTCSessionDescription = function (info) {
      ...
      return AdapterJS.WebRTCPlugin.plugin.ConstructSessionDescription(info.type, info.sdp);
    };
    ...
    attachMediaStream = function (element, stream, audio) {
      ...
      if (nodeName !== 'object') {
        ...
        temp.innerHTML = '<object id="' + elementId + '" ' + classHTML + 'type="' + AdapterJS.WebRTCPlugin.pluginInfo.type + '">' + '<param name="pluginId" value="' + elementId + '" /> ' + '<param name="pageId" value="' + AdapterJS.WebRTCPlugin.pageId + '" /> ' + '<param name="windowless" value="true" /> ' + '<param name="streamId" value="' + streamId + '" /> ' + '<param name="tag" value="' + tag + '" /> ' + '</object>';
        ...
        // already an <object> tag, just change the stream id
    ...
    reattachMediaStream = function (to, from) {
      ...
      for (var i = 0; i !== children.length; ++i) {
        if (children[i].name === 'streamId') {
          AdapterJS.WebRTCPlugin.WaitForPluginReady();
          stream = AdapterJS.WebRTCPlugin.plugin.getStreamWithId(AdapterJS.WebRTCPlugin.pageId, children[i].value);
          break;
        }
      }
      if (stream !== null) {
    ...
    RTCIceCandidate = function (candidate) {
      ...
      AdapterJS.WebRTCPlugin.WaitForPluginReady();
      return AdapterJS.WebRTCPlugin.plugin.ConstructIceCandidate(candidate.sdpMid, candidate.sdpMLineIndex, candidate.candidate);
    };
  ...
  AdapterJS.WebRTCPlugin.pluginNeededButNotInstalledCbPriv = function () {
    ...
    if (downloadLink) {
      ...
      if (AdapterJS.WebRTCPlugin.pluginInfo.portalLink) {
        // is portal link
        popupString = 'This website requires you to install the ' + ' <a href="' + AdapterJS.WebRTCPlugin.pluginInfo.portalLink + '" target="_blank">' + AdapterJS.WebRTCPlugin.pluginInfo.companyName + ' WebRTC Plugin</a>' + ' to work on this browser.';
      } else {
        // no portal link, just print a generic explanation
      ...
      // AdapterJS.renderNotificationBar(popupString, AdapterJS.TEXT.PLUGIN.BUTTON, downloadLink);
        ...
        // no download link, just print a generic explanation
...
},{"./CallStatistics":1,"./ChatView":2,"./PhoneRTCProxy":3,"./adapter":4,"./phonertc":6,"jquery":8}],6:[function(require,module,exports){
'use strict';
...
function Session(config) {
  ...
  if (typeof config.isInitiator === 'undefined' || typeof config.turn === 'undefined' || typeof config.streams === 'undefined') {
    throw {
      name: 'PhoneRTC Error',
...
},{"./PhoneRTCProxy":3,"bowser":7}],7:[function(require,module,exports){
/*!
